...
class RateStructure:
    """ RateStructure (container)
            contains all rates for a certain time t
        t       - time of the rate structure
        rates   - the forward rates at time t
    """
...
class SimResult:
    """ SimResult   (container)
            contains the simulated RateStructures for each time step
        __result    - a sorted list of RateStructures
    """
    def __init__(self):
        self.__result = []
    ...
    def __interpolate(self, time, timeInd, rateInd):
        ...
        if timeInd == 0:
            if time == self.__result[0].t:
                ...
                raise Exception(
                    "Trying to access " + str(time) + " while the earlist time point is " + str(self.__result[0].t))
            ...
            if time == self.__result[-1].t:
                ...
                raise Exception(
                    "Trying to access " + str(time) + " while the oldest time point is " + str(self.__result[0].t))
    ...
    def __sorted(self):
        """Check if self.__result is sorted or not"""
...
class vol6(volatility):
    ...
    def __str__(self):
        return "sigma(t) = [ a( T - t ) + d ] * exp( -b * ( T - t ) ) + c, a = " + str(self.a) + " b = " + str(
            self.b) + " c = " + str(self.c) + " d = " + str(self.d)

    def calibrate(self, tarVol, criticalTimePoint, tau):
        assert (len(tarVol) == len(criticalTimePoint) == len(tau))
        return minimize(self.__errToTarVol, [self.a, self.b, self.c, self.d], (tarVol, criticalTimePoint),
                        bounds=[(0.0, None), (0.0, None), (None, None), (None, None)], method='L-BFGS-B')
...
class vol7(volatility):
    ...
    def calibrate(self, tarVol, criticalTimePoint, tau):
        ...
        minimize(self.__errToTarVol, [self.a, self.b, self.c, self.d], (tarVol, criticalTimePoint),
                 bounds=[(0.0, None), (0.0, None), (None, None), (None, None)], method='L-BFGS-B')
    ...
    def __str__(self):
        return "sigma(t) = phi(i) * [ a( T(i) - t ) + d ] * exp( -b * ( T(i) - t ) ) + c, a = " + str(
            self.a) + ", b = " + str(self.b) + ", c = " + str(self.c) + ", d = " + str(self.d) + "\nphi = " + str(
            self.__phi)
...
class parametricCorrelator(correlator):
    ...
    def calibrate(self, tarCorr, criticalTimePoint, tau):
        assert (isinstance(tarCorr, np.matrix) and tarCorr.shape[0] == tarCorr.shape[1] == len(criticalTimePoint))
        minimize(self.__errToTarCorr, self.getPara(), (tarCorr, criticalTimePoint), method='L-BFGS-B')

    def __errToTarCorr(self, x, tarCorr, ctp):
        self.setPara(x)
        return np.sum(np.square(tarCorr - self.get(0, ctp)))
...
class ShortJumper(LiborMarketModel):
    ...
    def __init__(self, iniState, volCalc, corrCalc, criticalTimePoint, tau, numeraireIndex, timeStep=0.01):
        ...
        self.__dt = ShortJumper.maxTimeStep if (timeStep > ShortJumper.maxTimeStep) else         (
        timeStep if timeStep > 0 else 0.01)
    ...
    def simulate(self, finishTime):
        ...
        while (curRate.t <= finishTime):
            ...
            curRate = RateStructure(curRate.t + self.__dt,
                                    [prodIfNotNone(ele[0], ele[1]) for ele in zip(curRate, temp)])
            res.add(curRate)
